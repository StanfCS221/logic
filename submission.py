...
def formula2d():
    ...
    return Forall('$x', Forall('$y', Equiv(Grandmother('$y','$x'), And(Female('$x'),Exists('$z', And(Parent('$z','$x'), Parent('$y','$z')))))))
    # END_YOUR_CODE
...
def liar():
    ...
    j = AndList([TellTruth(john), Not(TellTruth(susan)),Not(TellTruth(mark)),Not(TellTruth(nicole))])
    s = AndList([Not(TellTruth(john)), TellTruth(susan),Not(TellTruth(mark)),Not(TellTruth(nicole))])
    m = AndList([Not(TellTruth(john)), Not(TellTruth(susan)), TellTruth(mark), Not(TellTruth(nicole))])
    n = AndList([Not(TellTruth(john)), Not(TellTruth(susan)), Not(TellTruth(mark)), TellTruth(nicole)])
    formulas.append(OrList([j,s,m,n]))

    j = AndList([CrashedServer(john), Not(CrashedServer(susan)), Not(CrashedServer(mark)), Not(CrashedServer(nicole))])
    s = AndList([Not(CrashedServer(john)), CrashedServer(susan), Not(CrashedServer(mark)), Not(CrashedServer(nicole))])
    m = AndList([Not(CrashedServer(john)), Not(CrashedServer(susan)), CrashedServer(mark), Not(CrashedServer(nicole))])
    n = AndList([Not(CrashedServer(john)), Not(CrashedServer(susan)), Not(CrashedServer(mark)), CrashedServer(nicole)])
    formulas.append(OrList([j, s, m, n]))
...
def ints():
    ...
    #formulas.append(Forall('$x', Exists('$y', Forall('$z', Implies(AndList([Successor('$x','$y'), Not(Equals('$x','$y')), Not(Equals('$z','$y'))]), Not(Successor('$x','$z')) )))))
    #formulas.append(Forall('$x', Exists('$y', Forall('$z', And( And(And(Successor('$x','$y'), Not(Equals('$y','$z'))),Not(Equals('$x','$z'))),Not(Successor('$x','$z')) )))))


    formulas.append(Forall('$x', And(Exists('$y', And(Successor('$x','$y'), Not(Equals('$x','$y')))) ,Forall('$y', Implies(And(Successor('$x','$y'),Not(Equals('$x','$y'))),Forall('$z', Equiv(Not(Equals('$z','$y')),Not(Successor('$x','$z')))))) ) ))
